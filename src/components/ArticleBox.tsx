...
export default function ArticleBox(props: { article: IArticle, index: number, className?: string }) {
    return (
        <div className={`flex flex-wrap bg-neutral-800 sm:flex-nowrap w-full ${props.className}`} key={`article-${props.index}`}>
            ...
            <div className={' py-3 px-4 border-r border-t border-b border-neutral-700 rounded-r flex flex-col'}>
                ...
                <Link to={`/article/${props.article._id}`}><h3 className={'text-3xl hover:text-white/60 cursor-pointer mb-1'}>{props.article.title}</h3></Link>
